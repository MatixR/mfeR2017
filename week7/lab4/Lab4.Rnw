...
Where $B_T\sim\mathcal{N}(0,T)$ is a normal random variable with zero mean zero and \emph{variance} equal to $T$. 

Given that a call option pays off, $\max\{ 0,S_{T}-K \}$, we can evalute the
...
\begin{equation*}
\mathbb{E} \left[ e^{-rT} \max \{ 0, S_T - K \} \right]
\end{equation*}
